...
def main():
    ...
    fltr_group.add_argument("-f", "--filter", metavar="CQL2 Filter", help="CQL2 filter expression. IMPORTANT: Must be wrapped in quotes! Example: --filter \"bldg_usage = '1100'\". Use the feature and attribute names described in the advise.yml file.")
    fltr_group.add_argument("--sql-filter", metavar="SQL Filter", help="The SQL Filter accepts WHERE conditions just as they are written in standard SQL queries. For example: bldg_usage = ‘residential’ AND bldg_height__measured_height > 12.5")
